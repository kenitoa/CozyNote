...
public final class MainController {
    ...
    private static final double LEFT_PANEL_WIDTH = 320;
    private static final double RIGHT_PANEL_WIDTH = 420;
    private static final double EDITOR_MIN_WIDTH = 760;
    private static final double BODY_PADDING = 18;
    private static final double BODY_SPACING = 16;
    ...
    public void fitInitialLayoutToWidth(double windowWidth) {
        if (windowWidth < requiredWindowWidth() && appState.leftSidebarVisibleProperty().get()) {
            appState.leftSidebarVisibleProperty().set(false);
        }
        updateResponsiveLayout(Math.max(0, windowWidth - (BODY_PADDING * 2)));
    }
    ...
    private void updateResponsiveLayout(double bodyWidth) {
        appBody.setSpacing(BODY_SPACING);
        appBody.setPadding(new Insets(BODY_PADDING));
        resizeSidePanel(sidebarPane, LEFT_PANEL_WIDTH);
        resizeSidePanel(rightPanelPane, RIGHT_PANEL_WIDTH);
        editorPane.setMinWidth(EDITOR_MIN_WIDTH);
        editorPane.setMaxWidth(Double.MAX_VALUE);
    }

    private double requiredWindowWidth() {
        double width = BODY_PADDING * 2 + EDITOR_MIN_WIDTH;
        int visiblePanels = 1;
        if (appState.leftSidebarVisibleProperty().get()) {
            width += LEFT_PANEL_WIDTH;
            visiblePanels++;
        }
        if (appState.rightPanelVisibleProperty().get()) {
            width += RIGHT_PANEL_WIDTH;
            visiblePanels++;
        }
        return width + (Math.max(0, visiblePanels - 1) * BODY_SPACING);
    }
